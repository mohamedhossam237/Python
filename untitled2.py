...
me = {
    0: {'user': 'ayda_sameh_' , 'password': 'password'}, 
    
}

url = 'https://instagram.com/'

accounts = [ 'duha_mo7sen']
...
def get_posts(link, posts, attempts):
    try:
        ...
        for (var i =0; i < """+ str(posts)  + """; i++) { 
            links += document.getElementsByClassName('eLAPa kPFhm')[i].children[0].href + '||';
            } 
        return links;
        """
...
def write_comment(post, comment , attempts):
    try:
        ...
        comment_bar = driver.find_element_by_class_name(' PUqUI Ypffh')
        comment_bar.click()
        time.sleep(3)
        comment_bar = driver.find_element_by_class_name('PUqUI Ypffh')
        comment_bar.click()
